...
print('Custom Edges: ',edges) # Note that the edgesare the same but the underlaying orbitals have changed
...
mol = sn.Molecule(geometry=geometry, basis_set='sto-3g', backend='pyscf')
mol3 =  sn.CLPO.generate_HAO_molecule(mol,use_active=False) # Note that for this purpose, is active space is set off
sn.plot_MO(mol3,filename='HAO',orbital=[3,8,13,18])
...
mol1,edges = sn.CLPO.generate_CLPO_molecule_edges(mol, edges=[(3,8),(13,18)], rm_files = False, use_active = True)
sn.plot_MO(mol1,'CLPO')
print('Edges ',edges)

# Please note that if the keyword rm_files is set to False on generate_CLPO_molecule_edges, the .molden files will be stored
