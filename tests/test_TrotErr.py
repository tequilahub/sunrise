...
# tequila.trotter_err is only available in newer tequila releases
EstTrotErr = pytest.importorskip("tequila.trotter_err.main_trot").EstTrotErr
...
def cartan_to_fermionic_operator(cobt, ctbts, orb_rot):
    ...
    tbts = np.einsum("ipa, iqb, irc, isd, iabcd -> ipqrs", orb_rot[1:], orb_rot[1:], orb_rot[1:], orb_rot[1:], ctbts)
...
def test_LR():
    ...
    assert np.isclose(alpha, 0.42178077369, atol=1.0e-4)
